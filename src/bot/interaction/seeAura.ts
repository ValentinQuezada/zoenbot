...
const seeAura_simple = async ({ userId, replyFn }: SeeAuraParams) => {
  await databaseConnection();
  const AuraPoints = mongoose.model("AuraPoints", AuraPointsSchema);
  ...
  const userAura = leaderboard.find(row => row.userId === userId) as any;
  if (userAura) {
    let privateMessage = `🔎 **Tus Aura Points (💠) por atributo:**\n`;
    ATTRIBUTES.forEach(attr => {
      if (attr.key !== "totalPoints") {
    ...
    privateMessage += `💠 **TOTALES: ${userAura.totalPoints}** Aura Points\n`;
    privateMessage += `📊​ Ranking: ${lastRank}/${leaderboard.length}`;
    await replyFn(privateMessage);
  } else {
    await replyFn("📂​ No hay datos de **Aura Points** aún.");
    return;
  }
};

export default {seeAuraCommand, seeAura_simple};